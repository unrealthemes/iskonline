...
{
	static public function dataAddress()
    {
	if(isset($_GET['address']))
		{
			//echo 'adress data push '.$_GET['address'];
			$token = "1689fbde959d4bae73931d54024ec2056fca3b74";
			//$secret = null;
			//$dadata = new \Dadata\DadataClient($token, $secret);
			$dadata = new \Dadata\DadataClient($token, null);
			$result1 = $dadata->suggest("address", $_GET['address'],15);
			$str='';
			
			//echo json_encode($result1);
			
			foreach($result1 as $ind=>$result)
			{	
					$str.='<div class="adr-item d-flex px-3 align-items-center" adr="'.$result['value'].'" region="'.$result['data']['region'].'" city="'.$result['data']['city'].'" onclick="adr_click(\''.$result['unrestricted_value'].'\')" >
					<span style="font-size: 16px;">'.$result['unrestricted_value'].'<br/></span>
				</div>';
				
				if(count($result1)-1!=$ind)
				{
					//$str.='<div class="rpy-1"><hr></div>';
				}
